...
class TeleconferenceRoomController
{
    ...
    public function dispatch($action, $queryVars)
    {
        ...
        if ($action == 'get_telehealth_launch_data') {
            ...
            return $this->getTranslationsAction($queryVars);
        } else {
...
//            if (!$this->appointmentService->isValidAppointmentStatus($status)) {
    ...
    public function getTranslationsAction($queryVars)
    {
        $translationController = new TeleHealthTranslationController($this->twig);
        echo $translationController->renderClientSideTranslations();
    }
