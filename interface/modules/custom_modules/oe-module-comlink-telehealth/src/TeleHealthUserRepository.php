...
class TeleHealthUserRepository extends BaseService
{
    ...
    public function saveUser(TeleHealthUser $user)
    {
        ...
        $binds = [$user->getUsername(), $user->getAuthToken(), $active];

        if (empty($user->getId())) {
            $sql = "INSERT INTO " . self::TABLE_NAME . "(`date_updated`, `username`,`auth_token`"
                . ",`active`, `user_id`,`patient_id`, `date_registered`) VALUES (NOW(),?,?,?,?,?,?)";
        } else {
            $sql = "UPDATE " . self::TABLE_NAME . " SET `date_updated`=NOW(),`username`=?,`auth_token`=?,`active`=?,`user_id`=?,`patient_id`=? WHERE `id`=?";
        }
