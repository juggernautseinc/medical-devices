...
class RCFaxClient extends AppDispatch
{
    ...
    public function sendSMS($tophone = null, $subject = null, $message = null, $from = null)
    {
        if ($this->authenticate() !== 1) {
            $e = xlt('Error: Authentication Service Denies Access. Not logged in. 1');
    ...
    {
        error_log("RC Authentication requested " . time(), 0);
        ...
        if ($this->platform->loggedIn()) {
            $logged_in = 1;
            error_log("Logged into RC " . time(),0);
            try {
        ...
        error_log("Stored authentication data " . time(), 0);
    ...
    public function getStoredDoc()
    {
        ...
        $uri = $docuri;
        if ($this->authenticate() !== 1) {
            $e = xlt('Error: Authentication Service Denies Access. Not logged in.c2');
            if ($this->authenticate() === 2) {
                $e = xlt('Error: Application account credentials is not setup. Setup in Actions->Account Credentials.');
            }
            $ee = array('error' => $e);
    ...
    public function viewFax()
    {
        ...
        if ($this->authenticate() !== 1) {
            $e = xlt('Error: Authentication Service Denies Access. Not logged in. 3');
            if ($this->authenticate() === 2) {
                $e = xlt('Error: Application account credentials is not setup. Setup in Actions->Account Credentials.');
            }
            $ee = array('error' => $e);
    ...
    {
        if ($this->authenticate() !== 1) {
            $e = xlt('Error: Authentication Service Denies Access. Not logged in. 4');
            if ($this->authenticate() === 2) {
                $e = xlt('Error: Application account credentials is not setup. Setup in Actions->Account Credentials.');
            }
            $ee = array('error' => $e);
    ...
    public function getCallLogs()
    {
        ...
        if ($this->authenticate() !== 1) {
            $e = xlt('Error: Authentication Service Denies Access. Not logged in. 5');
            if ($this->authenticate() === 2) {
                $e = xlt('Error: Application account credentials is not setup. Setup in Actions->Account Credentials.');
            }
            $ee = array('error' => $e);
    ...
    public function getPending()
    {
        ...
        error_log("Ringcentral info", $dateFrom . " " . $dateTo);
        if ($this->authenticate() !== 1) {
